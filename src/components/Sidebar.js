...
import MyNews from '../icons/my-news';
import Dashboard from '../icons/home';

import { menuSuperUser } from '../utils/menuSuperUser';
...
function Sidebar({
  ...
  const getIconMenu = (name) => {
    switch (name) {
      case 'users':
        return <User pathname={pathname} />
      case 'sections':
        return <Sections pathname={pathname} />
      case 'news':
        return <News pathname={pathname} />
      case 'profile':
        return <Profile pathname={pathname} />
      case 'myNews':
        return <MyNews pathname={pathname} />
      case 'dashboard':
    ...
    <div>
      ...
      >
        ...
        <div className="flex justify-between mb-10 pr-3 sm:px-2">
          ...
          {/* Logo */}
          <NavLink exact to="home" className="block">
            ...
            <ul className="mt-3">
              {menu && menu.map((item, index) => (
                <li className={`px-3 py-2 rounded-sm mb-0.5 last:mb-0 ${pathname === '/' && 'bg-gray-900'}`}>
                  <NavLink exact to={item.path} className={`block text-gray-200 hover:text-white truncate transition duration-150 ${pathname === '/' && 'hover:text-gray-200'}`}>
                    <div className="flex items-center">
                      {getIconMenu(item.icon)}
                      <span className="text-sm font-medium ml-3 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">{item.name}</span>
                    </div>
                  </NavLink>
                </li>
              ))}
